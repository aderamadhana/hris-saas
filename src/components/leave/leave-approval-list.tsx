...
import { getLeaveType, formatLeaveDate } from "@/lib/leave-types";
    ...
    try {
      const res = await fetch("/api/leave/approve", {
        ...
        body: JSON.stringify({
          ...
          rejectedReason:
            actionType === "rejected" ? actionComments : undefined,
        }),
      });
            ...
            >
              ...
              {isExpanded && (
                <div className="border-t bg-gray-50 px-4 py-4 space-y-3 text-sm">
                  ...
                  {item.previousApprovals &&
                    item.previousApprovals.length > 0 && (
                      <div>
                        <p className="text-xs font-medium uppercase tracking-wide text-gray-400 mb-1">
                          Riwayat persetujuan
                        </p>
          ...
          <div className="w-full max-w-md rounded-xl border bg-white p-6 shadow-xl">
            ...
            <p className="text-sm text-gray-500 mb-4">
              ...
              {getLeaveType(actionTarget.leaveType)?.name ??
                actionTarget.leaveType}{" "}
              ({actionTarget.days} hari)
            </p>
