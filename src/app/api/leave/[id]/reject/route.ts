...
export async function POST(
  ...
  try {
    ...
    if (!['owner', 'admin', 'hr', 'manager'].includes(currentEmployee.role)) {
      return NextResponse.json(
        { error: 'Insufficient permissions to reject leave' },
        { status: 403 }
      )
    }
    ...
    if (leaveRequest.organizationId !== currentEmployee.organizationId) {
      return NextResponse.json(
        { error: 'Cannot reject leave from different organization' },
        { status: 403 }
      )
    }
    ...
    return NextResponse.json({
      success: true,
      data: updatedLeave,
      message: `Leave request rejected for ${leaveRequest.employee.firstName} ${leaveRequest.employee.lastName}`,
    })
  } catch (error: any) {
