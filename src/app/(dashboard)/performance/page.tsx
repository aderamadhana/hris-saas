...
import { PerformanceClient } from "@/src/components/performance/performance-client";
...
export default async function PerformancePage() {
  ...
  const employee = await prisma.employee.findUnique({
    ...
    select: { id: true, role: true, organizationId: true, firstName: true, lastName: true },
  });
  ...
  const allReviews = isHRAdmin
    ? await prisma.performanceReview.findMany({
        ...
        include: {
          cycle:    { select: { id: true, name: true, type: true, status: true } },
          reviewee: { select: { firstName: true, lastName: true, employeeId: true, position: true } },
          reviewer: { select: { firstName: true, lastName: true } },
          goals:    true,
        },
        orderBy: { createdAt: "desc" },
